...
class NewsTab(QWidget):
    ...
    def render_html(self):
        ...
        link_color = "#6EB5FF" if is_dark else "#007AFF"
        link_hover = "#8BC8FF" if is_dark else "#0056b3"
        border_color = "#505050" if is_dark else "#E5E7EB"
        bg_color = "#3A3A3C" if is_dark else "#FFFFFF"
        bg_gradient = "#2C2C2E" if is_dark else "#F8FAFC"
        bg_hover = "#4A4A4C" if is_dark else "#F0F9FF"
        read_bg = "#2C2C2E" if is_dark else "#F3F4F6"
        title_color = "#FFFFFF" if is_dark else "#111827"
        meta_color = "#A0A0A0" if is_dark else "#6B7280"
        desc_color = "#D0D0D0" if is_dark else "#4B5563"
        tag_bg = "#0A84FF" if is_dark else "#007AFF"
        tag_color = "#FFFFFF"
        
        # 새로운 CSS 변수들
        action_bg = "rgba(255, 255, 255, 0.08)" if is_dark else "rgba(0, 122, 255, 0.08)"
        action_hover = "rgba(255, 255, 255, 0.15)" if is_dark else "rgba(0, 122, 255, 0.15)"
        bookmark_bg = "#17A2B8" if is_dark else "#007AFF"
        bookmark_end = "#20C997" if is_dark else "#00C7BE"
        empty_bg = "rgba(255, 255, 255, 0.03)" if is_dark else "rgba(0, 0, 0, 0.02)"
        scrollbar_track = "#2C2C2E" if is_dark else "#F3F4F6"
        scrollbar_thumb = "#505050" if is_dark else "#D1D5DB"

        css = AppStyle.HTML_TEMPLATE.format(
            text_color=text_color, link_color=link_color, link_hover=link_hover,
            border_color=border_color, bg_color=bg_color, bg_gradient=bg_gradient,
            bg_hover=bg_hover, read_bg=read_bg, title_color=title_color, 
            meta_color=meta_color, desc_color=desc_color,
            tag_bg=tag_bg, tag_color=tag_color,
            action_bg=action_bg, action_hover=action_hover,
            bookmark_bg=bookmark_bg, bookmark_end=bookmark_end,
            empty_bg=empty_bg, scrollbar_track=scrollbar_track, 
