...
class TestPendingRestoreStrictPolicy(unittest.TestCase):
    def test_apply_pending_restore_falls_back_to_runtime_backup_dir_when_payload_path_is_stale(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            cfg = root / "config.json"
            db = root / "db.sqlite"
            pending = root / "pending_restore.json"
            runtime_backup_dir = root / "backups" / "backup_3"
            ...
            (runtime_backup_dir / cfg.name).write_text(
                json.dumps({"app_settings": {"x": 9}}, ensure_ascii=False),
                encoding="utf-8",
            )
            backup_db = runtime_backup_dir / db.name
            conn = sqlite3.connect(str(backup_db))
            try:
                conn.execute("CREATE TABLE IF NOT EXISTS t (v INTEGER)")
                conn.execute("DELETE FROM t")
                conn.execute("INSERT INTO t(v) VALUES (9)")
                conn.commit()
            finally:
                conn.close()
            ...
            pending.write_text(
                json.dumps(
                    {
                        "backup_name": "backup_3",
                        "backup_dir": str(root / "stale-backups"),
                        "restore_db": True,
                    },
                    ensure_ascii=False,
                ),
            ...
            self.assertEqual(loaded_cfg["app_settings"]["x"], 9)
            conn = sqlite3.connect(str(db))
            try:
                row = conn.execute("SELECT v FROM t LIMIT 1").fetchone()
            finally:
                conn.close()
            self.assertEqual(int(row[0]), 9)
