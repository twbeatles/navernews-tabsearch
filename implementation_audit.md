...
- `RISK-008` 적용 완료(2026-02-25): 기사 삭제 후 duplicate flag 무결성 복구(`delete_link`/일괄 삭제 후 재계산)
- `RISK-009` 적용 완료(2026-02-25): pending restore 엄격 실패 정책(`restore_db=true` + DB 백업 누락 시 실패/보류 유지)
- `RISK-010` 적용 완료(2026-02-25): 설정 로드 정규화 강화(타입/범위 clamp + bool/alert_keywords 보정)
- `RISK-011` 적용 완료(2026-02-25): 페이지 커서 영속화(`pagination_state`) 및 DB count fallback 제거
- 검증 상태: `python -m pytest -q` 기준 전체 통과
- 검증 테스트 추가(2026-02-25):
  - `tests/test_pending_restore_strict.py`
  - `tests/test_pagination_state_persistence.py`
  - `tests/test_db_queries.py` 삭제 후 duplicate flag 회귀 추가
  - `tests/test_import_settings_normalization.py` 로드 경로 정규화 회귀 추가

## 1) 점검 요약

- 본 문서는 기능 구현 관점에서 실제 코드와 문서의 불일치, 운영 중 장애 가능성, 테스트 미보장 영역을 정리한 점검 결과다.
- 원문은 구현 착수 전 점검 문서이며, 현재는 반영 완료 상태를 함께 추적한다.
...
## 3) 추가 필요 기능 제안 (원문/히스토리)

- 참고: 아래 제안 중 핵심 1차 범위(RISK-008~RISK-011)는 2026-02-25 기준 구현 완료.
...
## 6) 우선순위 백로그 (`P0` / `P1` / `P2`, 2026-02-25 기준 완료 상태)

### P0

- `RISK-001` 자동 새로고침 락 플래그 회복 보장. (완료)
- `RISK-005` 키워드 그룹 데이터 저장/백업 계약 정합화. (완료)

### P1

- `RISK-002` 리네임 후 `btn_load` 키워드 바인딩 정합화. (완료)
- `RISK-003` 제외어-only 입력 차단 및 호환 처리. (완료)
- `RISK-004` 페이지네이션 상태 추적 구조 도입. (완료)

### P2

- `RISK-006` 문서-코드 간격 정렬. (완료)
- `RISK-007` `minimize_to_tray` 동작 연결 또는 항목 정리. (완료)
- `RISK-008` 삭제 후 duplicate flag 무결성 보장. (완료)
- `RISK-009` pending restore 엄격 실패 정책 반영. (완료)
- `RISK-010` 설정 로드 정규화 강화. (완료)
- `RISK-011` 페이지 커서 영속화 및 fallback 제거. (완료)
