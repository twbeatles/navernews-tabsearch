...
- 업데이터로 설치한 실행 파일의 작업 표시줄·Alt-Tab 아이콘이 Qt 기본 아이콘으로 남던 문제를 수정했습니다. 시작 화면보다 먼저 앱 아이콘을 정하고, 창 클래스 아이콘을 실행 파일에 들어 있는 아이콘으로 맞춥니다.
- Windows 알림용 앱 등록의 아이콘은 exe 옆에 ico가 없으면 실행 파일 자체를 사용합니다. 업데이터는 exe만 교체합니다.
- 실행 파일을 제자리에서 교체한 뒤 탐색기에 아이콘을 다시 읽도록 알립니다.
