...
- 참고 문서: `README.md`, `claude.md`, `project_structure_analysis.md`, `update_history.md`
- 참고: `implementation_functional_risk_review_2026-05-11.md`는 2026-05-11 기능 리스크 반영 후 삭제된 리뷰 스냅샷이며, 현재 작업트리에서도 삭제 상태로 유지한다.
...
> 2026-05-11 구현 반영: 아래 후보 중 규칙 기반 자동 분류/자동 액션, 전역 보관함 검색, 태그 관리 고도화와 일괄 태깅, Markdown 다이제스트 내보내기, 출처 alias 매핑은 2026-05-11 기능 리스크 배치에서 구현됐다. "오늘 볼 기사" 화면과 트렌드/기간 분석 대시보드 확장은 후속 후보로 남긴다.
...
반대로 외부 기사 원문 크롤링, LLM 요약, live DB 직접 공유형 클라우드 동기화 같은 기능은 매력은 있지만 보안/정책/의존성/키 관리/패키징 부담이 크므로 후순위가 맞다. 다만 클라우드 동기화는 2026-05-10에 "로컬 DB + 스냅샷 ZIP 병합"으로 범위를 좁혀 v1을 구현했다.
...
- `core/_db_schema.py`
- `core/_db_queries.py`
- `core/db_mutations_support/` (`core._db_mutations` 호환 facade)
- `core/_db_analytics.py`
- `core/database.py`

### 2. UI facade 분리가 이미 되어 있다

`MainApp`과 `NewsTab`은 facade에 가깝고, 실제 기능은 `ui/main_window_support/`, `ui/main_window_fetch_support/`, `ui/main_window_io_support/`, `ui/news_tab_support/` 아래로 분리되어 있다. 기능을 추가할 때 거대한 단일 파일에 다시 몰아넣지 않고 작은 support module에 붙일 수 있다.

관련 진입점:

- `ui/main_window_fetch_support/`
- `ui/_main_window_analysis.py`
- `ui/main_window_io_support/`
...
- `ui/main_window_io_support/exports.py`와 `data_io.py`의 CSV/Markdown export 경로
...
> 2026-05-11 구현 반영: `core/automation_rules.py`, `core/config_store_support/impl.py`, `ui.dialogs_support.article_tools.AutomationRulesDialog`, `ui._main_window_analysis.py`에 1차 구현이 들어갔다. 기존 import 호환 경로(`core.config_store_impl`, `ui.dialogs`)도 유지한다. 조건은 텍스트 키워드/제외어/출처/탭 검색어로 제한했고, 동작은 태그 추가/북마크/읽음/`제외` 태그+읽음으로 구현했다.
...
> 2026-05-11 구현 반영: `DatabaseManager.search_archive()` / `count_archive()`와 `ArchiveSearchDialog`가 추가됐다. 제목/요약, 메모, 태그, 출처 alias, 날짜, 북마크/미읽음 조건 조합을 지원한다.
...
> 2026-05-11 구현 반영: `TagManagerDialog`와 `DatabaseManager`의 rename/delete/bulk add/remove 경로가 추가됐다. 일괄 작업은 현재 로드 slice가 아니라 현재 탭의 전체 필터 scope 링크 목록을 기준으로 수행한다.
...
> 2026-05-11 구현 반영: `Ctrl+S` 저장 필터에 Markdown digest가 추가됐고, 현재 탭 전체 필터 scope를 UTF-8 `.md`로 원자 저장한다. HTML/PDF와 예약 발송은 후속 후보로 남긴다.
...
- 현재 구현은 `ui/main_window_io_support/exports.py`의 Markdown export helper를 사용하고, `ui/_main_window_settings_io.py`는 호환 facade로 남는다. 별도 `ui/exporters.py` 분리는 포맷이 늘어날 때 검토한다.
...
> 2026-05-11 구현 반영: `core/publisher_aliases.py`와 `PublisherAliasDialog`가 추가됐다. 원본 `publisher`는 DB에서 보존하고, 표시/통계/필터/아카이브/Markdown export에서 대표 출처명을 계산한다.
...
> 2026-05-10 구현 반영: 이 항목은 live SQLite DB를 클라우드 폴더에서 직접 공유하는 방식은 계속 금지하고, 대신 로컬 DB + 검증된 스냅샷 ZIP 병합 방식으로 v1을 완료했다.
...
- `core/cloud_sync.py`가 `manifest.json`, secret 제거 `settings.json`, SQLite backup API 복사 DB를 담은 `news_scraper_sync_*.zip`을 만든다.
- `core/_db_cloud_sync.py`가 기사/검색범위를 `link`, `(link, query_key)` 기준으로 union 병합한다.
- 읽음/북마크/메모/태그는 `read_updated_at`, `bookmark_updated_at`, `notes_updated_at`, `news_tag_state.tags_updated_at` 기준으로 최신 변경을 선택한다.
- `DATA_DIR` 또는 live DB가 OneDrive/Google Drive 같은 동기화 폴더 안에 있으면 자동 동기화를 차단한다.
- API 자격증명과 PC별 cloud folder path는 snapshot/settings export에 포함하지 않는다.
- 삭제 전파는 v1에서 제외한다.

#### 권장

v1의 구현 범위는 "데이터 손실을 피하는 보수적 병합"으로 유지한다. 다음 단계에서 삭제 전파가 필요해지면 tombstone 테이블과 per-device conflict 정책을 별도로 설계해야 한다.
...
| 기능 | 핵심 파일 | 테스트 후보 |
|---|---|---|
| 자동 분류/액션 | `core/config_store_support/impl.py`, `core/db_mutations_support/`, `core/automation_rules.py`, `ui/dialogs_support/article_tools.py`, `ui/_main_window_analysis.py` | `tests/test_functional_risk_20260511.py`, import/export roundtrip |
| 보관함 검색 | `core/_db_queries.py`, `ui/dialogs_support/article_tools.py`, `ui/_main_window_analysis.py` | `tests/test_functional_risk_20260511.py`, pagination, 액션 동기화 |
| 태그 관리 | `core/db_mutations_support/`, `core/_db_analytics.py`, `ui/dialogs_support/article_tools.py` | `tests/test_functional_risk_20260511.py`, scope 일괄 태그 |
...
| Markdown digest | `ui/main_window_io_support/exports.py`, `ui/main_window_io_support/data_io.py` | `tests/test_functional_risk_20260511.py`, escaping, cancel cleanup |
| 출처 alias | `core/publisher_aliases.py`, `core/config_store_support/impl.py`, card render/관리 UI | `tests/test_functional_risk_20260511.py`, alias display, import merge, filter 의미 보존 |
...
| 클라우드 스냅샷 동기화 | `core/cloud_sync.py`, `core/_db_cloud_sync.py`, `ui/main_window_io_support/cloud.py`, `ui/_settings_dialog_content.py` | `tests/test_cloud_sync.py`, settings roundtrip, path overlap block |
...
장기적으로는 `오늘 볼 기사` 큐와 `트렌드 분석`을 붙이면, 많은 탭과 긴 사용 기간에서 앱의 존재 이유가 더 강해진다. 다만 프로필, 원문 스냅샷, LLM 요약, 삭제 전파를 포함한 고급 클라우드 동기화는 안정성/정책/보안 비용이 크므로 현재 단계에서는 별도 설계 문서를 먼저 만드는 것이 맞다.
