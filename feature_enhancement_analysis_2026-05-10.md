...
- 참고 문서: `README.md`, `claude.md`, `project_structure_analysis.md`, `update_history.md`, `implementation_functional_risk_review_2026-05-11.md`
...
> 2026-05-11 구현 반영: `core/automation_rules.py`, `core/config_store_impl.py`, `ui.dialogs.AutomationRulesDialog`, `ui._main_window_analysis.py`에 1차 구현이 들어갔다. 조건은 텍스트 키워드/제외어/출처/탭 검색어로 제한했고, 동작은 태그 추가/북마크/읽음/`제외` 태그+읽음으로 구현했다.
...
- 현재 구현은 `ui/_main_window_settings_io.py`의 Markdown export helper를 사용한다. 별도 `ui/exporters.py` 분리는 포맷이 늘어날 때 검토한다.
...
| 기능 | 핵심 파일 | 테스트 후보 |
|---|---|---|
| 자동 분류/액션 | `core/config_store_impl.py`, `core/_db_mutations.py`, `core/automation_rules.py`, `ui/dialogs.py`, `ui/_main_window_analysis.py` | `tests/test_functional_risk_20260511.py`, import/export roundtrip |
| 보관함 검색 | `core/_db_queries.py`, `ui/dialogs.py`, `ui/_main_window_analysis.py` | `tests/test_functional_risk_20260511.py`, pagination, 액션 동기화 |
| 태그 관리 | `core/_db_mutations.py`, `core/_db_analytics.py`, `ui/dialogs.py` | `tests/test_functional_risk_20260511.py`, scope 일괄 태그 |
...
| Markdown digest | `ui/_main_window_settings_io.py` | `tests/test_functional_risk_20260511.py`, escaping, cancel cleanup |
| 출처 alias | `core/publisher_aliases.py`, `core/config_store_impl.py`, card render/관리 UI | `tests/test_functional_risk_20260511.py`, alias display, import merge, filter 의미 보존 |
...
| 클라우드 스냅샷 동기화 | `core/cloud_sync.py`, `core/_db_cloud_sync.py`, `ui/_main_window_settings_io.py`, `ui/_settings_dialog_content.py` | `tests/test_cloud_sync.py`, settings roundtrip, path overlap block |
